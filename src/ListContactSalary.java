...
class ListContactSalaryTabel extends JFrame{
    private JButton homeButton;
    private JTable tblCustomerDetails;
    private DefaultTableModel dtm;
    homePage HomePage;
    ListContactNameTabel listContactNameTabel;
    ...
    ListContactSalaryTabel(){
        ...
        JLabel UpdateContactTitle=new JLabel("LIST Contact By Salary");
    ...
    public  void  sortSalary(){
        ...
        for(int x=0;x<contactAdd.Contactinfro.length;x++){
            for(int i=1;i<contactAdd.Contactinfro.length-x;i++){
                if (contactAdd.Contactinfro[i-1].getCsalary().compareTo(contactAdd.Contactinfro[i].getCsalary()) > 0) {
...
public class ListContactSalary{
    public static void main(String [] args){
        new ListContactSalaryTabel().setVisible(true);
    }
}

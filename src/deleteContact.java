...
class ContactDelete extends JFrame{
    ...
    ContactDelete(){
        ...
        JLabel UpdateContactTitle=new JLabel("DELETE CONTACT");
    ...
    public boolean deleteCon(int index){
        ContactAdd[] tempContact = new ContactAdd[contactAdd.Contactinfro.length-1];
        for(int i=0;i<contactAdd.Contactinfro.length-1;i++){
            if(i==index){
                continue;
            }
            tempContact[i]=contactAdd.Contactinfro[i];
        }
        contactAdd.Contactinfro = tempContact;
        System.out.println("delete length is "+ contactAdd.Contactinfro.length);
        return true;
    }
...
public class deleteContact{
    public static void main(String args[]){
        new ContactDelete().setVisible(true);
